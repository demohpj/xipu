...
int8_t  STORAGE_Inquirydata[] =  /* 36 */
{
  ...
  0x00,
  0x00,
  'X', 'I', 'R', 'X', ' ', ' ', ' ', ' ', /* Manufacturer : 8 bytes */
  'X', 'i', 'P', 'C', ' ', 'S', 't', 'o', /* Product      : 16 Bytes */
  'r', 'a', 'g', 'e', ' ', ' ', ' ', ' ',
  '1', '.', '0' ,' ',                     /* Version      : 4 Bytes */
};
                    ...
                    uint32_t blk_addr, uint16_t blk_len)
{
  ...
  {
    if(!flashRead((blk_addr + i), (buf + (FLASH_SECTOR_SIZE * i))))
    {
      return(USBD_FAIL);
    }
  }
                     ...
                     uint32_t blk_addr, uint16_t blk_len)
{
  for(uint32_t i = 0; i < blk_len; i++)
  {
    if(!flashWrite((blk_addr + i), (buf + (FLASH_SECTOR_SIZE * i))))
    {
      return(USBD_FAIL);
    }
  }
