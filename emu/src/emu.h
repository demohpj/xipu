...
class Emu : public QMainWindow
{
	...
	private:
		...
		bool loadMemoryFile(const QString &path, CPU::Memory &memory);

		void romSetPage(int page);
		...
		int romPage; //!< Number of the ROM page to show
	...
	private slots:
		...
		void on_romGoToOSButton_clicked();
		void on_romGoToAppButton_clicked();
		void on_romMinusButton_clicked();
		void on_romPlusButton_clicked();
