...
Emu::Emu(QWidget *parent) : QMainWindow(parent), ui(new Ui::Emu)
{
	...
	this->ui->fileRomPathLabel->setText("");
...
void Emu::reset()
{
	...
	this->ui->regSPLMaxValueLabel->setText("");
...
void Emu::update()
{
	if(!this->started)
	{
		if((!this->ui->fileUrom0PathLabel->text().isEmpty()) && (!this->ui->fileUrom1PathLabel->text().isEmpty()) && (!this->ui->fileRomPathLabel->text().isEmpty()) && (!this->ui->fileFSDirPathLabel->text().isEmpty()))
		{
			this->loaded = true;
		}
		else
	...
	this->ui->fileRomOpenButton->setEnabled(!this->started);
	...
	this->ui->fileRomReloadButton->setEnabled((!this->started) && (!this->ui->fileRomPathLabel->text().isEmpty()));
	...
	this->ui->romGoToOSButton->setEnabled(this->started);
	this->ui->romGoToAppButton->setEnabled(this->started);
...
/**
 * Set ROM page to show
 *
 * @param page Page number
 */
void Emu::romSetPage(int page)
{
	if((page >= 0) && (((CPU::MEMORY_SIZE * 2) / CPU::MEMORY_PAGE_SIZE) > page))
	{
		this->romPage = page;

		this->ui->romText->clear();

		if(this->started)
		{
			const CPU::Memory &rom = (((CPU::MEMORY_SIZE / CPU::MEMORY_PAGE_SIZE) > page) ? this->cpu.getRom() : this->cpu.getRamHigh());
			...
			for(int i = 0; i < CPU::MEMORY_PAGE_SIZE; i++)
			{
				...
				if((CPU::MEMORY_SIZE / CPU::MEMORY_PAGE_SIZE) > page)
				{
					text.append(QString("%1").arg(rom.data[(this->romPage * CPU::MEMORY_PAGE_SIZE) + i], 2, 16, QChar('0')));
				}
				else
				{
					text.append(QString("%1").arg(rom.data[((this->romPage - (CPU::MEMORY_SIZE / CPU::MEMORY_PAGE_SIZE)) * CPU::MEMORY_PAGE_SIZE) + i], 2, 16, QChar('0')));
				}
			}

			this->ui->romText->setPlainText(text);

			this->ui->romMinusButton->setEnabled(page != 0);
			this->ui->romPlusButton->setEnabled(page < (((CPU::MEMORY_SIZE * 2) / CPU::MEMORY_PAGE_SIZE) - 1));
		}
		else
		{
			this->ui->romMinusButton->setEnabled(false);
			this->ui->romPlusButton->setEnabled(false);
		}
	}

	QString addressPrefix = QString("%1").arg(this->romPage, 2, 16, QChar('0')).toUpper();
	QString address = "";
	...
	this->ui->romAddressLabel->setText(QString().append(address));
}
...
void Emu::ramSetPage(int page)
{
	if((page >= 0) && ((CPU::MEMORY_SIZE / CPU::MEMORY_PAGE_SIZE) > page))
	{
		...
		if(this->started)
		{
			const CPU::Memory &ram = this->cpu.getRamLow();
			...
			for(int i = 0; i < CPU::MEMORY_PAGE_SIZE; i++)
			{
				...
				text.append(QString("%1").arg(ram.data[(this->ramPage * CPU::MEMORY_PAGE_SIZE) + i], 2, 16, QChar('0')));
			}
...
/**
 * Load data from file to the memory buffer
 *
 * @param path Path to the file to load
 * @param memory Buffer of the memory to fill
 *
 * @return Status of loading data to the memory buffer
 */
bool Emu::loadMemoryFile(const QString &path, CPU::Memory &memory)
{
	QFile file(path);
	...
	if(CPU::MEMORY_SIZE != file.read(reinterpret_cast<char *>(memory.data.data()), CPU::MEMORY_SIZE))
	{
...
void Emu::updateSlot()
{
	...
	this->ui->regCValueLabel->setText(reg.c ? QString("1") : QString("0"));
	this->ui->regZValueLabel->setText(reg.z ? QString("1") : QString("0"));
	...
	this->ui->regSPLMaxValueLabel->setText(QString("%1").arg(reg.maxSpl, 2, 16, QChar('0')));

	this->romSetPage(this->romPage);
...
void Emu::on_fileUrom0OpenButton_clicked()
{
	...
	if(!path.isEmpty())
	{
		...
		if(this->loadMemoryFile(path, urom0))
		{
...
void Emu::on_fileUrom1OpenButton_clicked()
{
	...
	if(!path.isEmpty())
	{
		...
		if(this->loadMemoryFile(path, urom1))
		{
...
void Emu::on_fileRomOpenButton_clicked()
{
	QString path = QFileDialog::getOpenFileName(this, "", "", "ROM (*.bin)");

	if(!path.isEmpty())
	{
		CPU::Memory rom;

		if(this->loadMemoryFile(path, rom))
		{
			this->ui->fileRomPathLabel->setText(path);
...
void Emu::on_fileUrom0ReloadButton_clicked()
{
	...
	if(this->loadMemoryFile(this->ui->fileUrom0PathLabel->text(), urom0))
	{
		this->cpu.setUrom0(urom0);
	}
	else
...
void Emu::on_fileUrom1ReloadButton_clicked()
{
	...
	if(this->loadMemoryFile(this->ui->fileUrom1PathLabel->text(), urom1))
	{
		this->cpu.setUrom1(urom1);
	}
	else
...
void Emu::on_fileRomReloadButton_clicked()
{
	CPU::Memory rom;

	if(this->loadMemoryFile(this->ui->fileRomPathLabel->text(), rom))
	{
		this->cpu.setRom(rom);
	}
	else
...
//! Process go to the OS section for ROM event
void Emu::on_romGoToOSButton_clicked()
{
	this->romSetPage(CPU::ROM_OS_ADDRESS / CPU::MEMORY_PAGE_SIZE);
}

//! Process go to the application section for ROM event
void Emu::on_romGoToAppButton_clicked()
{
	this->romSetPage(CPU::ROM_APP_ADDRESS / CPU::MEMORY_PAGE_SIZE);
}

//! Process go to the previous page for ROM event
void Emu::on_romMinusButton_clicked()
{
	this->romSetPage(this->romPage - 1);
}

//! Process go to the next page for ROM event
void Emu::on_romPlusButton_clicked()
{
	this->romSetPage(this->romPage + 1);
}
