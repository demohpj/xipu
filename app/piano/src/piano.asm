...
const tone2x  0, 17,  0,  0, 22, 24,  0,  0, 31,  0, 29,  0,  0,  0,  0,  0
const tone3x 32, 16, 18, 20, 21, 23, 25, 27, 28, 30,  0,  0,  0,  0,  0,  0
const tone4x 19,  0, 74, 70, 55, 38,  0, 58, 60, 46, 62,  0, 65,  0,  0, 48
const tone5x 50, 34,  0, 53, 41,  0, 72, 36,  0, 43, 67,  0,  0,  0, 26,  0
const tone6x  0, 51, 73, 69, 54, 37, 56, 57, 59, 45, 61, 63, 64, 76, 75, 47
const tone7x 49, 33, 39, 52, 40, 44, 71, 35, 68, 42, 66,  0,  0,  0,  0,  0
...
.printKeyHelp:
	...
	mov low(*keyHelpHeader), X
	mov high(*keyHelpHeader), Y
	; Print start message
	call .xStringPrint
	call .xStringNewLine
	; Load address to 
	mov low(*keyHelp00), X
	mov high(*keyHelp00), Y
	; Reset loop counter
	clr B
.printKeyHelp_loop:
	; New line
	...
	and 0x1, A
	cmp 0x1, A
	jne .printKeyHelp_loop_white
	; Set foregound color to green
	mov 0x0a, A
	call .xLcdSetForeground
	jmp .printKeyHelp_loop_print
.printKeyHelp_loop_white:
	; Set foreground color to default
	mov 0x0f, A
	call .xLcdSetForeground
.printKeyHelp_loop_print:
	; Print line
	call .xStringPrint
	...
	and 0x3, A
	cmp 0x3, A
	jne .printKeyHelp_loop_noNewLine
	; New line if it is a keyboard row was printed
	call .xStringNewLine
.printKeyHelp_loop_noNewLine:
	...
	mov low(*keyHelpExit), X
	mov high(*keyHelpExit), Y
...
.printMs:
	...
	mov low(*keyHelpClear), X
	mov high(*keyHelpClear), Y
